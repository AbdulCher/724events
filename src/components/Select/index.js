...
const Select = ({
  ...
  const changeValue = (newValue) => {
    onChange(newValue); // Etait vide
    setValue(newValue);
    setCollapsed(true); // Pas la bonne valeur
  };

  return (
    <div className={`SelectContainer ${type}`} data-testid="select-testid">
      {/* Affiche le label SEULEMENT si ce n'est pas "Toutes" */}
      {label && label !== "Toutes" && <div className="label">{label}</div>}

      <div className="Select">
        <ul>
          <li className={collapsed ? "SelectTitle--show" : "SelectTitle--hide"}>
            {/* Affiche "Toutes" uniquement ici */}
